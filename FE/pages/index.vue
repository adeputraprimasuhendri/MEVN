  ...
  <div class="container">
    <div class="mt-5">
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tr v-for="user in datauser">
            ...
            <td>{{user.email}}</td>
            <td>********</td>
            <th class="text-center"><button class="btn btn-sm btn-danger" @click="deleteuser(user._id)">Delete</button>
            </th>
          </tr>
        </table>
      </div>
      <b-modal id="modal-1" title="Form Member BootstrapVue" hide-footer="true">
        ...
        <b-button variant="info" class="btn-block" @click="adduser">Saves</b-button>
      </b-modal>
    </div>
  </div>
</template>
